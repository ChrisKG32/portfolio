...
const getPackageManagerCommand = (packageManager) =>
  ...
  ({
    ...
    pnpm: () => {
      ...
      const includeDoubleDashBeforeArgs = semver.lt(
        pnpmVersion,
        '7.0.0',
      )
...
const main = async ({ packageManager, rootDirectory }) => {
  ...
  const EXAMPLE_ENV_PATH = path.join(
    rootDirectory,
    '.env.example',
  )
  ...
  const [appArc, env, readme, packageJson] =
    await Promise.all([
...
`
  ...
  await Promise.all([
    ...
    fs.rm(
      path.join(rootDirectory, '.github', 'ISSUE_TEMPLATE'),
      {
        recursive: true,
      },
    ),
    ...
    fs.rm(
      path.join(rootDirectory, '.github', 'dependabot.yml'),
    ),
    fs.rm(
      path.join(
        rootDirectory,
...
async function askSetupQuestions({
  ...
  console.log(
    `✅  Project is ready! Start development with "${pm.run(
      'dev',
    )}"`,
  )
}
